...
class MemberService(
    ...
    fun memberSave(memberDTO: JoinMemberDTO): Member {
        ...
        val member = Member(id = memberDTO.accountId, mbti = convertMember.mbti, nickName = convertMember.nickName)
    ...
    fun updateMemberProfileImage(memberId: Long, profileImage: ProfileImage): Member {
        val updated = repository.updateByProfileImage(memberId, profileImage)
    ...
    fun followUser(myId: Long, followId: Long): Follow {
        ...
        val follow = Follow(follower = my, following = following)
        val savedFollow = followRepository.save(follow)
        my.followings?.add(savedFollow) // 내 followings에 추가
        following.followers?.add(savedFollow) // 팔로우한 사람의 followers에 추가
        return savedFollow
    }
    ...
    fun getRecommendUser():List<Member>{
        return repository.findRandomAccount()
    }
}